...
const NewNoteModal = () => {
  ...
  const handleAddNote = async (event) => {
    ...
    try {
      ...
      showToast("Failed to create new note. please try again later.", "error");
    }
  };
  ...
  return showNewNoteForm ? (
    <div className="new-note-container flex-col flex-align-center flex-justify-center p-2">
      ...
      >
        ...
        {formDataError && (
          <p className="text-lg error-color mt-1-5 mx-auto">{formDataError}</p>
        )}
        <div className="note-actions flex-row flex-justify-center flex-align-center flex-wrap">
          <div className="note-action-wrapper">
            ...
            {showColorPalette && (
              <ColorPalette
                handleChangeNoteBackgroundColor={handleNoteItemChange}
                noteBackgroundColor={noteItem.noteBackgroundColor}
