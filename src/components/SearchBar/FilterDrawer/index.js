...
const FilterDrawer = ({ handleChangeShowFilterModal }) => {
  ...
  const handleFilterByLabelChange = (labelId) => {
    const newFilterByLabels = filterByLabel.map((filter) =>
      filter.id === labelId ? { ...filter, filtered: !filter.filtered } : filter
    );
      ...
      <div className="filter-sort-drawer py-0-25" ref={filterSortRef}>
        <div className="filter-sort-drawer-head py-0-75 px-0-75 flex-row flex-align-center flex-justify-between">
          ...
          <button
            ...
            onClick={(e) => handleChangeShowFilterModal("CLOSE_FILTER_MODAL")}
          >
            <Close color="white" />
          </button>
        </div>
        <div className="filter-sort-options py-0-75 px-0-75 flex-col flex-align-start flex-justify-center">
          <div className="sort-options">
            ...
            <select
              ...
              value={sortByDate}
            >
              <option value="" name="sortByDate" defaultValue disabled>
                --Chose an option--
              </option>
          ...
          <div className="sort-options">
            ...
            >
              <option value="" name="sortByPriority" defaultValue disabled>
                --Chose an option--
              </option>
          ...
          {labels.length > 0 && (
            <div className="filter-options">
              <p className="filter-head text-reg">Filter By Labels</p>
                ...
                {labels.map(({ label, id }) => (
                  ...
                  >
                    <input
                      ...
                      value={label}
                      checked={
                        filterByLabel.find((filter) => filter.id === id)
                          .filtered
                      }
